...
export function returnEntryChunkData(bundle) {
    const entryChunkData = [];
...
export async function generateHtmlPages(outDir, base, pageTemplateFilePath, pageMetaMapFilePath, generatedEntryChunks) {
    // ensure output directory exists
    const resolvedOutDir = path.resolve(outDir);
    fileUtils.ensureDirExists(resolvedOutDir);
    // generate temp output directory path for bundled files
    const tempBundleOutputDir = path.join(resolvedOutDir, `temp-bundle-${Date.now()}`);
    // bundle user files
    await build({
    ...
    pages.forEach((page) => {
        let html = ReactDOMServer.renderToString(React.createElement(PageTemplate, page));
        // check if the generated HTML contains ' id="root"'
        if (!html.includes(' id="root"')) {
            throw new Error(`Error: The vite-plugin-react-meta-map's PageTemplate does not contain ' id="root"'. Ensure that the PageTemplate renders an element with id="root".`);
        }
        // ensure that `page` contains `url` and `bundleEntryPoint` properties
        if (!("url" in page)) {
            throw new Error(`Error: The vite-plugin-react-meta-map's pageMetaMap does not contain the "url" property. The "url" property is required to know each page's relative .html file path (ie: "index.html").`);
        }
        if (!("bundleEntryPoint" in page)) {
            throw new Error(`Error: The vite-plugin-react-meta-map's pageMetaMap does not contain the "bundleEntryPoint" property. The "bundleEntryPoint" property is required to know which bundle entry file to load for each page (ie: "/src/main.tsx").`);
        }
        ...
        generatedEntryChunks.forEach((chunk) => {
            ...
            if (isMatchingEntryPoint) {
                ...
                assetTags.push(`<script type="module" crossorigin src="${base}${chunk.fileName}"></script>`);
                // include preload links for imports
                for (const importFile of chunk.imports) {
                    assetTags.push(`<link rel="modulepreload" crossorigin href="${base}${importFile}">`);
                }
                // include CSS links
                for (const cssFile of chunk.css) {
                    assetTags.push(`<link rel="stylesheet" crossorigin href="${base}${cssFile}">`);
                }
            }
        });
        // ensure page's entry point file found
        if (!entryPointmatchFound) {
            throw new Error(`Error: "bundleEntryPoint" set for ${page.url} in the vite-plugin-react-meta-map's pageMetaMap could not be found in the Vite bundle as an entry point. Ensure that the "bundleEntryPoint" is set to an existing bundle entry point file (ie: "/src/main.tsx").`);
        }
...
//# sourceMappingURL=metaMapPluginLogic.js.map